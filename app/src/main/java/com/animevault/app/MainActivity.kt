...
class MainActivity : android.app.Activity() {
    ...
    private val textColor = Color.rgb(244, 246, 251)
    ...
    private fun buildNavigation() {
        ...
        val items = listOf(
            Triple("🏠", "Dashboard", { showDashboard() }),
            Triple("📅", "Schedule", { showSchedule() }),
            Triple("📚", "Watchlist", { showList("Watchlist", "watchlist") }),
            Triple("✅", "Completed", { showList("Completed", "completed") }),
            Triple("⭐", "Favorites", { showList("Favorites", "favorites") }),
            Triple("🔥", "Ongoing", { showList("Ongoing", "ongoing") }),
            Triple("🔒", "Mature", { showMature() }),
            Triple("⚙", "Settings", { showSettings() })
        )
        items.forEach { (icon, label, action) ->
            val b = navButton(icon, label, width)
    ...
    private fun showBrowse() {
        ...
        val search = EditText(this).apply {
            hint = "Search anime..."; setTextColor(textColor); setHintTextColor(muted); setPadding(dp(14), 0, dp(14), 0)
            background = rounded(panel2, 14)
        }
    ...
    private fun showDashboard() {
        ...
        stats.forEach { stat ->
            ...
            val v = TextView(this).apply { text = stat.second; textSize = 24f; setTextColor(textColor) }
            card.addView(v)
            c.addView(card)
        }
        title(c, "Watch Time")
    ...
    private fun showSettings() {
        ...
        for (i in 1..8) {
            val name = EditText(this).apply { hint = if (i == 8) "Provider 8 (18+)" else "Provider $i name"; setTextColor(textColor); setHintTextColor(muted); background = rounded(panel2, 12) }
            val url = EditText(this).apply { hint = "Provider $i URL"; setTextColor(textColor); setHintTextColor(muted); background = rounded(panel2, 12); inputType = android.text.InputType.TYPE_TEXT_VARIATION_URI }
    ...
    private fun toggleRow(c: LinearLayout, label: String, state: Boolean, onChange: (Boolean) -> Unit) {
        ...
        val t = TextView(this).apply { text = label; textSize = 16f; setTextColor(textColor) }
    ...
    private fun simpleRow(t:String):TextView=TextView(this).apply{text=t;setTextColor(textColor);textSize=15f;setPadding(16,16,16,16);background=rounded(panel2,14);layoutParams=LinearLayout.LayoutParams(-1,dp(62)).apply{setMargins(0,5,0,5)}}
    private fun title(c:LinearLayout,t:String){c.addView(TextView(this).apply{text=t;textSize=24f;setTextColor(textColor);setTypeface(null,android.graphics.Typeface.BOLD);setPadding(0,14,0,6)})}
    ...
    private fun button(t:String, color:Int)=Button(this).apply{text=t;textSize=13f;setTextColor(textColor);background=rounded(color,14);stateListAnimator=null}
